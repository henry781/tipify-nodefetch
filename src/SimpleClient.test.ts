...
import {isString, JsonConverter, jsonObject, jsonProperty} from 'tipify';
import {SimpleClient, SimpleClientOptions} from './SimpleClient';
import {SimpleClientError} from './SimpleClientError';

const simpleClientOptions: SimpleClientOptions = {
    fetch: fetch as any,
...
describe('SimpleClient', () => {
    ...
    const client = new SimpleClient(simpleClientOptions);
    const principal = {
        getToken() {
            return {token: 'okfopsdfsdfsd5f4sd6f4sd65f', scheme: 'bearer', params: undefined};
        },
        getClientHeaders() {
            return {
    ...
    describe('get', () => {
        ...
        describe('with authentication', () => {
            ...
            it('should set auth header from user', async () => {
                ...
                // TEST
                await client.get(fullPath, {mode: 'json', user: principal});
            ...
            it('should set auth header from token', async () => {
                ...
                // TEST
                await client.get(fullPath, {mode: 'json', user: principal, token: 'token'});
        ...
        describe('when nok', () => {
            ...
            it('should return error when status is not expected', async () => {
                ...
                try {
                    ...
                    // VERIFY
                    expect(e.name).equal('SimpleClientError');
